...
Tracing comes from `symfony/ai-open-telemetry-bridge` (path repository on `../src/open-telemetry-bridge`), there is no telemetry code in `src/`. The `tracing` section of `config/packages/ai.yaml` makes the AI Bundle decorate every platform, agent, toolbox and retriever and export via its `exporter` shortcut to `OTEL_EXPORTER_OTLP_ENDPOINT` (a no-op while empty), authenticated with `LANGFUSE_AUTH`. The `langfuse-*` services of `compose.yaml` are a local Langfuse v4 (UI on :3000, project and keys created on boot); `.env.test` blanks the endpoint so tests never export. The exporter is set to `protocol: 'http/json'`, since Langfuse's protobuf response body breaks the PHP exporter.
