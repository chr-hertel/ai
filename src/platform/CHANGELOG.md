 ...
 * Verify a replayed HTTP cassette request signature before serving the recorded response; an encoded JSON body also signs in its decoded form, so a bridge that switched between handing the payload over as a string and as an array still replays its cassettes
