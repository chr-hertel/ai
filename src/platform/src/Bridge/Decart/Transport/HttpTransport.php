...
final class HttpTransport implements TransportInterface
{
    public function __construct(
        ...
        private readonly string $baseUrl = 'https://api.decart.ai/v1',
    ) {
    }

    public function send(Model $model, RequestEnvelope $request, array $options = []): RawResultInterface
    {
        return new RawHttpResult($this->httpClient->request($request->getMethod(), rtrim($this->baseUrl, '/').$request->getPath(), [
