...
use Symfony\AI\Platform\Bridge\Higgsfield\GenerationClient;
...
final class GenerationClientTest extends TestCase
{
    private const BASE_URL = 'https://platform.higgsfield.ai';

    public function testSupportsModel()
    {
        $client = new GenerationClient(new MockHttpClient([], self::BASE_URL), new MockClock());
    ...
    public function testConvertReturnsTheDownloadedMediaAsBinaryResult()
    {
        $httpClient = new MockHttpClient([
            new MockResponse('video-bytes', ['response_headers' => ['content-type' => 'video/mp4']]),
        ], self::BASE_URL);

        $client = new GenerationClient($httpClient, new MockClock());

        $result = $client->convert(new RawHttpResult($httpClient->request('GET', 'https://cdn.higgsfield.ai/video.mp4')));

        $this->assertInstanceOf(BinaryResult::class, $result);
        $this->assertSame('video-bytes', $result->getContent());
    ...
    public function testClientPollsUntilCompletion()
    {
        ...
        $client = new GenerationClient($httpClient, $clock, 5);
