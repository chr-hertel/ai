...
final class GenerateContentClientTest extends TestCase
{
    ...
    {
        $converter = new GenerateContentClient(new HttpTransport(new MockHttpClient(), 'unused'));
    ...
    public function testRequest()
    {
        ...
        self::httpClient($httpClient)->request(new Gemini('gemini-1.5-flash'), $payload);
    }
    ...
    public function testRequestWithOptions()
    {
        ...
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options) {
            ...
            $this->assertSame(['temperature' => 0.7, 'topP' => 0.9], $body['generationConfig']);
        ...
        self::httpClient($httpClient)->request(new Gemini('gemini-1.5-flash'), ['contents' => []], $options);
    }

    public function testRequestMergesGenerationConfigOptionAndKeepsRequestFieldsTopLevel()
    {
        ...
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options) use ($safetySettings) {
            ...
            $this->assertSame(['temperature' => 0.7, 'topK' => 3], $body['generationConfig']);
            $this->assertSame($safetySettings, $body['safetySettings']);
            $this->assertSame('cachedContents/abc', $body['cachedContent']);
    ...
    public function testRequestPutsStructuredOutputSchemaIntoGenerationConfig()
    {
        ...
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options) use ($schema) {
            ...
            $this->assertSame(['responseMimeType' => 'application/json', 'responseJsonSchema' => $schema], $body['generationConfig']);

            return new JsonMockResponse(['candidates' => []]);
        });

        self::httpClient($httpClient)->request(new Gemini('gemini-1.5-flash'), ['contents' => []], [
            PlatformSubscriber::RESPONSE_FORMAT => ['json_schema' => ['schema' => $schema]],
        ]);
    }

    public function testRequestWrapsStringPayloadIntoUserContents()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options) {
            $body = json_decode($options['body'], true);

            $this->assertSame([['role' => 'user', 'parts' => [['text' => 'Hello, world!']]]], $body['contents']);

            return new JsonMockResponse(['candidates' => []]);
        });

        self::httpClient($httpClient)->request(new Gemini('gemini-1.5-flash'), 'Hello, world!');
    }
    ...
    public function testRequestWithStreamUsesDifferentEndpoint()
    {
        ...
        self::httpClient($httpClient)->request(new Gemini('gemini-1.5-flash'), ['contents' => []], ['stream' => true]);
    }
    ...
    public function testThrowsOnUnhandledErrorStatusBeforeStreaming()
    {
        ...
        $httpResponse->method('getStatusCode')->willReturn(403);
        $httpResponse->method('getContent')->willReturn('{"error":{"message":"Forbidden"}}');

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Unexpected response code 403: "{"error":{"message":"Forbidden"}}"');

        (new GenerateContentClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert(new RawHttpResult($httpResponse), ['stream' => true]);
    }

    public function testItReturnsAggregatedTextOnSuccess()
    {
        $result = (new GenerateContentClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert($this->codeExecutionResult('code_execution_outcome_ok.json'));
        $this->assertInstanceOf(MultiPartResult::class, $result);
    ...
    {
        $result = (new GenerateContentClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert($this->codeExecutionResult('code_execution_outcome_failed.json'));
        $this->assertInstanceOf(MultiPartResult::class, $result);
    ...
    {
        $result = (new GenerateContentClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert($this->codeExecutionResult('code_execution_outcome_deadline_exceeded.json'));
        $this->assertInstanceOf(MultiPartResult::class, $result);
    ...
    public function testStreamingYieldsTokenUsageWhenUsageMetadataIsPresent()
    {
        ...
        $result = (new GenerateContentClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert($rawResult, ['stream' => true]);
    ...
    private function codeExecutionResult(string $fixture): RawHttpResult
    {
        $response = $this->createStub(ResponseInterface::class);
        $response->method('toArray')->willReturn(json_decode(file_get_contents(__DIR__.'/CodeExecution/Fixtures/'.$fixture), true));

        return new RawHttpResult($response);
    }

    private static function httpClient(MockHttpClient $httpClient): GenerateContentClient
    {
        return new GenerateContentClient(new HttpTransport($httpClient, 'test-api-key'));
    }
}
