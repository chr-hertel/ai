...
final class GenerateContentClientTest extends TestCase
{
    ...
    /**
     * The conversion side of the client is transport-independent; a stub keeps
     * these tests focused on payload shaping.
     */
    private static function client(): GenerateContentClient
    {
        return new GenerateContentClient(new class implements TransportInterface {
            public function send(Model $model, RequestEnvelope $request, array $options = []): RawResultInterface
            {
                throw new \LogicException('The transport is not exercised by conversion tests.');
            }
        });
    }
}
