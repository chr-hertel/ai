 ...
 */
final class GenerateContentClient implements ApiClientInterface
{
    ...
    private const REQUEST_FIELDS = ['cachedContent', 'labels', 'safetySettings', 'systemInstruction', 'toolConfig'];
    ...
    public function request(Model $model, array|string $payload, array $options = []): RawResultInterface
    {
        ...
        $path = \sprintf('models/%s:%s', $model->getName(), $options['stream'] ?? false ? 'streamGenerateContent?alt=sse' : 'generateContent');
        ...
        $generationConfig = array_merge(
            (array) ($options['generationConfig'] ?? []),
            array_diff_key($options, array_flip(['generationConfig', 'stream', 'tools', 'tool_config', 'server_tools', ...self::REQUEST_FIELDS])),
        );

        $config = [] === $generationConfig ? [] : ['generationConfig' => $generationConfig];
        $config += array_intersect_key($options, array_flip(self::REQUEST_FIELDS));
        ...
        return $this->transport->send($path, array_merge($config, $payload));
    }
    ...
    public function convert(RawResultInterface $result, array $options = []): ResultInterface
    {
        ...
        if (isset($data['error'])) {
            throw new RuntimeException(\sprintf('Error "%s" - "%s": "%s".', $data['error']['code'], $data['error']['status'], $data['error']['message']), $data['error']['code']);
        }
