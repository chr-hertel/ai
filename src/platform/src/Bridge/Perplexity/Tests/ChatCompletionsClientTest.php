...
use Symfony\AI\Platform\Bridge\Perplexity\ChatCompletionsClient;
...
final class ChatCompletionsClientTest extends TestCase
{
    ...
    public function testThrowsServerExceptionOnServerErrorStatusBeforeStreaming()
    {
        ...
        $converter->convert($converter->request(new Perplexity('sonar'), ['messages' => []], ['stream' => true]), ['stream' => true]);
    }

    public function testThrowsOnUnhandledHttpErrorStatusBeforeStreaming()
    {
        $converter = self::createClient(new MockHttpClient(new MockResponse('Forbidden', ['http_code' => 403])));

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Unexpected response code 403: "Forbidden"');
    ...
    {
        $this->assertTrue(self::createClient()->supports(new Perplexity('sonar')));
    }

    public function testUsesPerplexityTokenUsageExtractor()
    {
        $this->assertInstanceOf(TokenUsageExtractor::class, self::createClient()->getTokenUsageExtractor());
    }
    ...
    {
        return new ChatCompletionsClient(new HttpTransport($httpClient, 'https://api.perplexity.ai', 'pplx-api-key'), '/chat/completions', Perplexity::class, false);
    }
}
