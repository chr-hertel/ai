...
final class MessagesClientTest extends TestCase
{
    ...
    {
        $modelClient = self::createClient(new MockHttpClient());
    ...
    public function testItAuthenticatesWithApiKeyAndWorkspaceHeaders()
    {
        ...
        $modelClient = self::createClient(new MockHttpClient($responseCallback), cacheRetention: 'none', workspace: 'proj_example');
    ...
    public function testItSignsAnthropicHeadersWithSigV4()
    {
        ...
        $modelClient = new MessagesClient(
            new MessagesTransport(
    ...
    public function testItAddsPromptCachingMarkers()
    {
        ...
        $modelClient = self::createClient(new MockHttpClient($responseCallback), cacheRetention: 'long');
    ...
    private static function createClient(MockHttpClient $httpClient, string $cacheRetention = 'short', ?string $workspace = null): MessagesClient
    {
        return new MessagesClient(
            new MessagesTransport($httpClient, 'https://bedrock-mantle.us-east-1.api.aws', 'us-east-1', 'bedrock-api-key', workspace: $workspace),
            $cacheRetention,
        );
    }
}
