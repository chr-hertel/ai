 ...
 */
final class ImageGenerationClient implements ApiClientInterface
{
    ...
    public function request(Model $model, array|string $payload, array $options = []): RawResultInterface
    {
        ...
        return $this->transport->send('/v1/images/generations', array_merge($options, [
            'model' => $model->getName(),
            'prompt' => $payload,
        ]));
    }
     ...
     * @param array{image: ImageContent, ...} $options
     */
    private function edit(Model $model, string $prompt, array $options): RawResultInterface
    {
        $image = $options['image'];
        unset($options['image']);
        ...
        return $this->transport->send('/v1/images/edits', $body, ['Content-Type' => 'multipart/form-data; boundary='.$boundary]);
    }
}
