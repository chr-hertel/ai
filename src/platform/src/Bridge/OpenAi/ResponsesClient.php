...
final class ResponsesClient implements EndpointClientInterface
{
    ...
    /**
     * GPT speaks both the Responses and the Chat Completions contract; the
     * provider registers this client first, so it stays the default and
     * chat completions is reachable via \$options['endpoint'].
     */
    /**
     * Azure serves the same contract with its own model classes, so the family
     * is a provider decision rather than a fact of the contract.
     */
