...
use Symfony\AI\Platform\Bridge\OpenResponses\ResponsesClient;
...
final class ResponsesClientTest extends TestCase
{
    public function testItWrapsHttpClientInEventSourceHttpClient()
    {
        ...
        $modelClient = new ResponsesClient($httpClient, 'https://api.example.com');
    ...
    {
        $modelClient = new ResponsesClient(new MockHttpClient(), 'https://api.example.com');
    ...
    public function testItIsExecutingTheCorrectRequest()
    {
        ...
        $modelClient = new ResponsesClient($httpClient, 'https://api.example.com', 'test-api-key');
        $modelClient->request(
    ...
    public function testItWorksWithoutApiKey()
    {
        ...
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ResponsesClient($httpClient, 'https://localhost:1234');
        $modelClient->request(
    ...
    public function testItUsesCustomResponsesPath()
    {
        ...
        $httpClient = new MockHttpClient([$resultCallback]);
        $modelClient = new ResponsesClient($httpClient, 'https://api.example.com', 'test-api-key', '/custom/responses');
        $modelClient->request(
            new ResponsesModel('test-model'),
            ['input' => [['role' => 'user', 'content' => 'Hello']]],
        );
    ...
    public function testItParsesHeaderlessSseStreams()
    {
        ...
        $modelClient = new ResponsesClient(new MockHttpClient([$response]), 'https://api.example.com', 'test-api-key');
