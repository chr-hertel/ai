...
final class HttpTransport implements TransportInterface
{
    ...
    /**
     * @param string $baseUrl Base URL of an Anthropic-compatible endpoint, with or without a trailing slash
     */
    ...
    public function send(Model $model, RequestEnvelope $request, array $options = []): RawResultInterface
    {
        ...
        if ($statusCode >= 500) {
            ...
            throw new ServerException($statusCode, $errorMessage);
        }

        if ($statusCode >= 400) {
            throw new RuntimeException(\sprintf('Unexpected response code %d: "%s"', $statusCode, $response->getContent(false)));
        }
