...
final class MessagesConvertTest extends TestCase
{
    ...
    public function testThrowsExceedContextSizeExceptionWhenPromptIsTooLong()
    {
        ...
        $this->sendThroughTransport($httpClient);
    }
    ...
    public function testThrowsServerExceptionOnServerErrorStatusBeforeStreaming()
    {
        ...
        try {
            $this->sendThroughTransport($httpClient, ['stream' => true]);
    ...
    public function testStreamingExposesStopReasonAsFinishReasonMetadata()
    {
        ...
        $streamResult = (new MessagesClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert($this->createRawResult($events), ['stream' => true]);
    ...
    {
        $searches = [];
        foreach ((new MessagesClient(new HttpTransport(new MockHttpClient(), 'unused')))->convert($raw, ['stream' => true])->getContent() as $delta) {
    ...
    /**
     * HTTP status handling moved from the result converter into the transport,
     * so these cases are exercised through an actual request.
     *
     * @param array<string, mixed> $options
     */
    private function sendThroughTransport(MockHttpClient $httpClient, array $options = []): void
    {
        $client = new MessagesClient(new HttpTransport($httpClient, 'unused'));
        $client->request(new Claude('claude-sonnet-4-5'), ['messages' => []], $options);
    }
