...
use Symfony\AI\Platform\Bridge\DeepSeek\ChatCompletionsClient;
...
final class ChatCompletionsClientTest extends TestCase
{
    ...
    public function testConvertThrowsExceedContextSizeExceptionOnContextOverflow()
    {
        ...
        $converter = self::createClient($httpClient);

        $converter->convert($converter->request(new DeepSeek('deepseek-chat'), []));
    }
    ...
    public function testConvertThrowsRuntimeExceptionWithBareMessageOnOtherErrorCodes()
    {
    ...
    public function testThrowsServerExceptionOnServerErrorStatusBeforeStreaming()
    {
        ...
        $converter->convert($converter->request(new DeepSeek('deepseek-chat'), [], ['stream' => true]), ['stream' => true]);
    }

    private static function createClient(MockHttpClient $httpClient = new MockHttpClient()): ChatCompletionsClient
    {
        return new ChatCompletionsClient(new HttpTransport($httpClient, 'https://api.deepseek.com', 'test-api-key'), '/chat/completions', DeepSeek::class, false);
    }
}
