...
use Symfony\AI\Platform\Bridge\Cerebras\ChatCompletionsClient;
...
final class ChatCompletionsClientTest extends TestCase
{
    ...
    public function testConvertThrowsExceedContextSizeExceptionOnContextOverflow()
    {
        ...
        $converter->convert($converter->request(new Model('gpt-oss-120b'), []));
    }
    ...
    public function testConvertThrowsOnEmptyChoices()
    {
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Response does not contain output.');
    ...
    public function testThrowsServerExceptionOnServerErrorStatusBeforeStreaming()
    {
        ...
        $converter->convert($converter->request(new Model('gpt-oss-120b'), [], ['stream' => true]), ['stream' => true]);
    }

    private static function createClient(MockHttpClient $httpClient = new MockHttpClient()): ChatCompletionsClient
    {
        return new ChatCompletionsClient(new HttpTransport($httpClient, 'https://api.cerebras.ai', 'csk-1234567890abcdef'), modelClass: Model::class, applyGatewayDefaults: false);
    }
}
