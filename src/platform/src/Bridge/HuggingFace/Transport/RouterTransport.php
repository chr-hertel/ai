...
final class RouterTransport implements TransportInterface
{
    ...
    /**
     * @param string $baseUrl Base URL of the HuggingFace router, with or without a trailing slash
     */
    ...
    public function send(Model $model, RequestEnvelope $request, array $options = []): RawResultInterface
    {
        ...
        $url = $this->baseUrl.'/'.ltrim(strtr($request->getPath(), ['{provider}' => $provider, '{name}' => $model->getName()]), '/');
