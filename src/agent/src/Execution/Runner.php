...
final class Runner
{
    ...
    public function run(string $model, MessageBag $messages, array $options): \Generator
    {
        ...
        while (true) {
            ...
            if (null === $toolCallResult || null === $this->toolExecutor) {
                // $metadata holds the previous rounds only, the final result carries its own
                yield new ResultUpdate($this->finalize($result, $metadata, $sources));
            ...
            if ($event->hasResult()) {
                yield new ResultUpdate($this->finalize($event->getResult(), $metadata, $sources));
    ...
    private function consumeStream(StreamResult $stream): \Generator
    {
        ...
        $result = [] !== $toolCalls ? new ToolCallResult($toolCalls) : new TextResult($text);
    ...
    private function finalize(ResultInterface $result, Metadata $metadata, SourceCollection $sources): ResultInterface
    {
