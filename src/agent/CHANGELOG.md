 ...
 * [BC BREAK] Change `AgentInterface::call()` to return a lazy, iterable `Execution` that is also the `ResultInterface` it produces — read it eagerly (`getContent()`, `await()`), iterate it to observe every model request, tool call and streamed delta as a `Progress` update, or register `onProgress()`/`onResult()` callbacks
