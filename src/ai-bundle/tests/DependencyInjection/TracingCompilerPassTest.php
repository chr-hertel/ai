...
final class TracingCompilerPassTest extends TestCase
{
    public function testProcessDecoratesTaggedServices()
    {
        ...
        $this->assertEquals([new Reference('.inner'), new Reference('ai.tracing.tracer'), 'openai', true], $platform->getArguments());
        ...
        $this->assertEquals([new Reference('.inner'), new Reference('ai.tracing.tracer'), true], $agent->getArguments());
     ...
     */
    private function container(?array $instrument, bool $captureContent): ContainerBuilder
    {
