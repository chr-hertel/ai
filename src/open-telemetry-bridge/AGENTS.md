...
- `user.id` is only recorded with a `UserIdResolverInterface`, on inference and agent spans
- `SemanticConvention\OpenInferenceAttributes` (`input.value`/`output.value` on agent spans) is a temporary workaround for the session view of Arize Phoenix; remove it once Phoenix derives them from `gen_ai.*.messages`
