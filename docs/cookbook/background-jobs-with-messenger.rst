...
exactly one request per call and never sleeps — and then decides what to do with the answer. The
bundle registers the client of every job-capable platform as ``ai.platform.job_client.<name>``, so
naming the argument after the platform is enough to get it injected::
    ...
    final class FinishVideoJobHandler
    {
        ...
        {
            $status = $this->minimaxJobClient->getStatus($message->handle);

            if ($status->is(JobStateCase::SUCCEEDED)) {
                $result = $this->minimaxJobClient->getResult($message->handle);
